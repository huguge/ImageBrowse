...
public class MainActivity extends ListActivity {
	...
	class MyTask extends AsyncTask<Void, Void, MyMessage> {
		...
		@Override
		protected MyMessage doInBackground(Void... params) {//后台执行，比较耗时的操作都可以放在这里。注意这里不能直接操作UI
			...
			Gson gson = new Gson();//用gson方式进行解析
			MyMessage msg = gson.fromJson(json, MyMessage.class);
			return msg;
		}

		@Override
		protected void onPostExecute(MyMessage result) {//用doInBackground得到的结果，更新ui，主线程执行
